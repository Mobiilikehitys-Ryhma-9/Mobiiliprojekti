...
export default function ProfileScreen({ navigation }: Props) {
  ...
  const menuItems = [
    { label: "Vaihda käyttäjänimi", onPress: () => setShowUsernameModal(true) },
    { label: "Vaihda salasana", onPress: () => setShowPasswordModal(true) },
    { label: "Vaihda profiilikuva", onPress: () => { } },
    { label: "Kirjaudu ulos", onPress: handleSignOut },
  ];
    ...
    <View style={styles.container}>
      {user ? (
        <View style={{ flex: 1, backgroundColor: "white" }}>
            ...
            <View>
              ...
              <View style={styles.likeContainer}>
                <MaterialIcons
                  ...
                  style={{ marginRight: 6 }} />
                <Text style={globalStyles.text}>{userData?.likes ?? 0}</Text>
              </View>
            </View>
          </View>
          ...
          {/* Asetusvalikko */}
          {userSettings && (
            <ModalWrapper title="Asetukset" onClose={() => setUserSettings(false)}>
              {menuItems.map((item) => (
                <TouchableOpacity
                  key={item.label}
